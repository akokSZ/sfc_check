...
@echo off >nul
 Chcp 866 >nul
  title Проверка целостности системных файлов / 0.8.0-beta  [ SafeZone.cc ]
...
if %ErrorLevel% EQU 5 ( 
		...
		echo.
		echo 101. Устранение неполадок, которые могут мешать правильной работе приложений из Microsoft Store
		echo 102. Поиск и исправление проблем с воспроизведением звука
		echo 103. Поиск и исправление проблем с записью звука	
		echo 104. Поиск и исправление проблем, которые могут мешать выполнению фонового скачивания файлов
		echo 105. Устранение проблем с устройствами и принтерами
		echo 106. Поиск и устранение проблем с оборудованием и устройствами
		echo 107. Поиск и устранение проблем с производительностью Internet Explorer
		echo 108. Поиск и устранение проблем со средствами безопасности и конфиденциальности в Internet Explorer	
		echo 109. Поиск и очистка неиспользуемых файлов и ярлыков, а также выполнение задач по обслуживанию
		echo 110. Поиск и исправление проблем с подключением к Интернету или веб-сайтам
		echo 111. Поиск и устранение проблем с доступом к файлам и папкам на других компьютерах
		echo 112. Поиск и устранение проблем с просмотром компьютеров или общих файлов в домашней группе	
		echo 113. Поиск и устранение проблем с беспроводными и другими сетевыми адаптерами
		echo 114. Поиск и устранение проблем со входящими подключениями компьютеров и брандмауэром Windows
		echo 115. Поиск и устранение проблем с подключением к рабочей сети с помощью DirectAccess
		echo 116. Поиск и устранение проблем с запуском старых программ в этой версии Windows
		echo 117. Поиск и устранение проблем для оптимизации быстродействия и производительности Windows
		echo 118. Поиск и устранение проблем с параметрами электропитания компьютера для энергосбережения и увеличения времени работы батареи
		echo 119. Поиск и устранение проблем с печатью
		echo 120. Поиск и устранение проблем службы Windows Search
		echo 121. Поиск и устранение проблем с воспроизведением фильмов, телепередач и видео
		echo 122. Поиск и устранение проблем с параметрами проигрывателя Windows Media
		echo 123. Поиск и устранение проблем с библиотекой проигрывателя Windows Media
		echo 124. Поиск и устранение проблем с воспроизведением DVD-дисков в проигрывателе Windows Media
		echo 125. Устранение ошибок Центра обновления Windows
	pause 
	cls ) 
...
cls
call :secretlist "Анализ состояния хранилища компонентов" "Процедура очистки хранилища компонентов" "Процедура восстановления хранилища компонентов" "Создать точку восстановления" "Список доступных спецкоманд 77" "Выход"
